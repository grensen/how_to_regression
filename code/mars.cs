...
public class Program
{
    static void Main(string[] args)
    {
        ...
        Console.WriteLine("\nFirst three train X: ");
        for (int i = 0; i < 3; ++i) VecShow(dt.XTrain[i], 4, 8);
        Console.WriteLine("\nFirst three train y: ");
        for (int i = 0; i < 3; ++i)
        ...
        MARS mars = EvaluateModel(new MARS(),
            m => m.Train(dt.XTrain, dt.YTrain), dt.XTrain, dt.YTrain, dt.XTest, dt.YTest);

        Console.WriteLine($"\nFirst three predicted y = ");
        for (int i = 0; i < 3; i++)
            Console.Write($"{mars.Predict(dt.XTrain[i]),8:F4}");
        Console.WriteLine("");
    ...
    public static T EvaluateModel<T>(T model, Action<T> trainAction,
     float[][] trainingData, float[] labels, float[][] trainingData2, float[] labels2)
    {
        trainAction(model);

        dynamic dynamicModel = model; // Use dynamic to invoke methods on the model.
        Console.Write($"\n{dynamicModel.GetType().Name} model trained.\n");

        try
        {
            // Compute metrics
            var trainMSE = RootMSE(trainingData, labels, dynamicModel);
            var trainMAE = MAE(trainingData, labels, dynamicModel);
            var trainAcc1 = Accuracy(trainingData, labels, 0.01f, dynamicModel);
            var trainAcc5 = Accuracy(trainingData, labels, 0.05f, dynamicModel);
            var trainAcc10 = Accuracy(trainingData, labels, 0.1f, dynamicModel);

            var testMSE = RootMSE(trainingData2, labels2, dynamicModel);
            var testMAE = MAE(trainingData2, labels2, dynamicModel);
            var testAcc1 = Accuracy(trainingData2, labels2, 0.01f, dynamicModel);
            var testAcc5 = Accuracy(trainingData2, labels2, 0.05f, dynamicModel);
            var testAcc10 = Accuracy(trainingData2, labels2, 0.1f, dynamicModel);
            ...
            Console.WriteLine(
                $"\nTraining: E({trainMSE:F3}, {trainMAE:F3}), " +
                $"A({trainAcc10:F2}, {trainAcc5:F2}, {trainAcc1:F2}), " +
                $"S({scoreTrain:F3})" + $"" +
                $"\nTesting:  E({testMSE:F3}, {testMAE:F3}), " +
...
{
    // Hyperparameters
    public int MaxBasisFunctions { get; set; } = 15;
    public float MinGcvImprovement { get; set; } = 1e-4f;
    public float RegularizationLambda { get; set; } = 0.01f;  // Ridge penalty
    public bool EnablePruning { get; set; } = true;
    public int MaxInteractionLevel { get; set; } = 2;
    public bool StandardizeFeatures { get; set; } = true;
    public int MaxCandidateKnotsPerFeature { get; set; } = 120; // maximum candidate knots per feature
    public bool UseParallelCandidates { get; set; } = true;    // Optional: use parallel evaluation for candidates
    ...
    // Cache for basis function evaluations (keyed by unique candidate string)
    private Dictionary<string, float[]> evaluationCache;

    public MARS()
    {
        basisFunctions = new List<BasisFunction>();
        evaluationCache = new Dictionary<string, float[]>();
    }

    public void Train(float[][] data, float[] labels, bool info = true)
    {
        if (data == null || labels == null || data.Length == 0 || data.Length != labels.Length)
            throw new ArgumentException("Invalid training data or labels.");
        ...
        float[][] transformedData = data;
        if (StandardizeFeatures)
        {
            (featureMeans, featureStdDevs) = ComputeFeatureScaling(data);
            transformedData = StandardizeData(data, featureMeans, featureStdDevs);
        }

        // Start with the constant basis function.
        basisFunctions.Clear();
        basisFunctions.Add(new BasisFunction(BasisFunctionType.Constant));

        // Initial prediction is the mean of the labels.
        float initialPrediction = labels.Average();
        float[] currentPredictions = Enumerable.Repeat(initialPrediction, numSamples).ToArray();
        float currentRss = CalculateRss(labels, currentPredictions);
        float currentGcv = CalculateGcv(currentRss, basisFunctions.Count, numSamples);

        // Forward selection: add basis functions until no significant GCV improvement.
        for (int bfCount = 1; bfCount < MaxBasisFunctions; ++bfCount)
        {
            BasisFunction bestNewBf = null;
            float bestCandidateGcv = float.MaxValue;

            // Generate candidate basis functions.
            List<BasisFunction> candidateList = new List<BasisFunction>();

            // 1. Generate raw hinge candidates (with quantile-based knot selection).
            for (int featureIndex = 0; featureIndex < numFeatures; ++featureIndex)
            {
                List<float> candidateKnots = GetCandidateKnots(transformedData, featureIndex, MaxCandidateKnotsPerFeature);
                foreach (float knot in candidateKnots)
                    foreach (BasisFunctionType type in new[] { BasisFunctionType.HingePositive, BasisFunctionType.HingeNegative })
                        candidateList.Add(new BasisFunction(type, featureIndex, knot));
            }

            // 2. Generate interaction candidates: combine each raw candidate with each existing nonconstant basis.
            var rawCandidates = candidateList.ToList(); // copy of current raw candidates
            foreach (var rawCand in rawCandidates)
                // Combine only with existing nonconstant functions (skip constant).
                for (int i = 1; i < basisFunctions.Count; i++)
                {
                    var existing = basisFunctions[i];
                    if (existing.InteractionLevel + rawCand.InteractionLevel <= MaxInteractionLevel)
                        candidateList.Add(new BasisFunction(existing, rawCand));
                }

            // Evaluate each candidate.
            // Optionally, you can parallelize the loop below.
            IEnumerable<BasisFunction> candidateEnumerable = candidateList;
            if (UseParallelCandidates)
                candidateEnumerable = candidateList.AsParallel();

            foreach (var candidate in candidateEnumerable)
            {
                var tempBasisFunctions = new List<BasisFunction>(basisFunctions) { candidate };
                float[] tempCoeffs = EstimateCoefficients(transformedData, labels, tempBasisFunctions);
                float[] tempPredictions = CalculatePredictions(transformedData, tempBasisFunctions, tempCoeffs);
                float tempRss = CalculateRss(labels, tempPredictions);
                float tempGcv = CalculateGcv(tempRss, tempBasisFunctions.Count, numSamples);

                // Uncomment for debugging:
                // Console.WriteLine($"{candidate} => GCV: {tempGcv}");
                if (tempGcv < bestCandidateGcv)
                {
                    bestCandidateGcv = tempGcv;
                    bestNewBf = candidate;
                }
            }

            // Accept candidate if improvement is significant.
            if (bestNewBf != null && (currentGcv - bestCandidateGcv) > MinGcvImprovement)
            {
                // Console.WriteLine($"Adding {bestNewBf} improves GCV from {currentGcv} to {bestCandidateGcv}");
                basisFunctions.Add(bestNewBf);
                currentGcv = bestCandidateGcv;
            }
            else
            {
                break; // No candidate offers enough improvement.
            }
        }

        // Optional backward deletion (pruning) using GCV.
        if (EnablePruning && basisFunctions.Count > 1)
            PruneBasisFunctions(transformedData, labels);

        // Final coefficient estimation.
        coefficients = EstimateCoefficients(transformedData, labels, basisFunctions);

        void Info()
        {
            if (info)
            {
                Console.WriteLine($"\nMARS (Multivariate Adaptive Regression Splines) settings: ");
                Console.WriteLine("MaxBasisFunctions   = " + MaxBasisFunctions);
                Console.WriteLine("MinGcvImprovement   = " + MinGcvImprovement);
                Console.WriteLine("Regularization      = " + RegularizationLambda);
                Console.WriteLine("EnablePruning       = " + EnablePruning);
                Console.WriteLine("MaxInteractionLevel = " + MaxInteractionLevel);
                Console.WriteLine("StandardizeFeatures = " + StandardizeFeatures);
                Console.WriteLine("MaxCandidateKnots   = " + MaxCandidateKnotsPerFeature);
                Console.WriteLine("ParallelCandidates  = " + UseParallelCandidates);
            }
        }
    }
    ...
    public float Predict(float[] sample)
    {
        ...
        // Standardize the sample if needed.
        if (StandardizeFeatures)
            sample = StandardizeSample(sample, featureMeans, featureStdDevs);

        float prediction = 0f;
        for (int i = 0; i < basisFunctions.Count; i++)
            prediction += coefficients[i] * basisFunctions[i].Evaluate(sample);
        return prediction;
    }

    public float[] Predict(float[][] samples)
    {
        return samples.Select(Predict).ToArray();
    }

    #region Candidate & Matrix Utilities

    /// <summary>
    /// Returns candidate knot values for a feature.
    /// If there are too many distinct values, select evenly spaced quantiles.
    /// </summary>
    private List<float> GetCandidateKnots(float[][] data, int featureIndex, int maxKnots)
    {
        var distinct = data.Select(row => row[featureIndex]).Distinct().OrderBy(x => x).ToList();
        if (distinct.Count <= maxKnots)
            return distinct;
        List<float> knots = new List<float>();
        for (int k = 0; k < maxKnots; k++)
        {
            int idx = (int)Math.Round(k * (distinct.Count - 1) / (float)(maxKnots - 1));
            knots.Add(distinct[idx]);
        }
        return knots;
    }

    /// <summary>
    /// Computes RSS between actual and predicted.
    /// </summary>
    private float CalculateRss(float[] actual, float[] predicted)
    {
        float rss = 0f;
        for (int i = 0; i < actual.Length; i++)
            rss += (actual[i] - predicted[i]) * (actual[i] - predicted[i]);
        return rss;
    }

    /// <summary>
    /// Computes the GCV: GCV = RSS / (1 - (M/n))^2, where M = number of basis functions.
    /// </summary>
    private float CalculateGcv(float rss, int numBasis, int numSamples)
    {
        float penalty = 1 - (numBasis / (float)numSamples);
        if (penalty <= 0) penalty = 1e-6f;
        return rss / (penalty * penalty);
    }

    /// <summary>
    /// Constructs the design matrix (n x m) for the current basis functions,
    /// using cached evaluations.
    /// </summary>
    private float[][] GetDesignMatrix(float[][] data, List<BasisFunction> basisFuncs)
    {
        int n = data.Length;
        int m = basisFuncs.Count;
        float[][] designMatrix = new float[n][];
        // Precompute each column using caching.
        var columns = new List<float[]>();
        foreach (var bf in basisFuncs)
        {
            columns.Add(GetEvaluations(bf, data));
        }
        for (int i = 0; i < n; i++)
        {
            designMatrix[i] = new float[m];
            for (int j = 0; j < m; j++)
                designMatrix[i][j] = columns[j][i];
        }
        return designMatrix;
    }

    /// <summary>
    /// Estimates coefficients using regularized least squares (ridge regression).
    /// The constant function is not regularized.
    /// </summary>
    private float[] EstimateCoefficients(float[][] data, float[] labels, List<BasisFunction> basisFuncs)
    {
        int n = data.Length;
        int m = basisFuncs.Count;
        float[][] X = GetDesignMatrix(data, basisFuncs);
        float[][] Xt = MatrixTranspose(X);
        float[][] XtX = MatrixMultiply(Xt, X);

        // Add regularization (skip j==0).
        for (int i = 0; i < m; i++)
            XtX[i][i] += RegularizationLambda * (i == 0 ? 0 : 1);

        float[][] invXtX = MatrixInverse(XtX);
        if (invXtX == null)
            return Enumerable.Repeat(0f, m).ToArray();

        float[][] XtY = MatrixMultiply(Xt, VectorToMatrix(labels));
        float[][] coeffMatrix = MatrixMultiply(invXtX, XtY);
        return MatrixToVector(coeffMatrix);
    }

    /// <summary>
    /// Computes predictions given the design matrix and coefficients.
    /// </summary>
    private float[] CalculatePredictions(float[][] data, List<BasisFunction> basisFuncs, float[] coeffs)
    {
        int n = data.Length;
        float[][] X = GetDesignMatrix(data, basisFuncs);
    ...
    /// <summary>
    /// Backward deletion (pruning) using GCV as the criterion.
    /// </summary>
    private void PruneBasisFunctions(float[][] data, float[] labels)
    {
        int n = data.Length;
        bool improvement = true;
        while (improvement && basisFunctions.Count > 1)
        {
            improvement = false;
            int bestRemoveIndex = -1;
            float[] currentCoeffs = EstimateCoefficients(data, labels, basisFunctions);
            float currentGcv = CalculateGcv(CalculateRss(labels, CalculatePredictions(data, basisFunctions, currentCoeffs)), basisFunctions.Count, n);

            for (int i = 1; i < basisFunctions.Count; i++) // Skip constant.
            {
                var tempBasis = new List<BasisFunction>(basisFunctions);
                tempBasis.RemoveAt(i);
                float[] tempCoeffs = EstimateCoefficients(data, labels, tempBasis);
                float tempGcv = CalculateGcv(CalculateRss(labels, CalculatePredictions(data, tempBasis, tempCoeffs)), tempBasis.Count, n);
                if (tempGcv < currentGcv)
                {
                    currentGcv = tempGcv;
                    bestRemoveIndex = i;
                    improvement = true;
            ...
            if (bestRemoveIndex != -1)
                basisFunctions.RemoveAt(bestRemoveIndex);
        }
    }

    #endregion

    #region Matrix Utilities (with Partial Pivoting)

    private float[][] MatrixTranspose(float[][] matrix)
    {
        int rows = matrix.Length, cols = matrix[0].Length;
        float[][] trans = new float[cols][];
        for (int j = 0; j < cols; j++)
        {
            trans[j] = new float[rows];
    ...
    {
        float[][] mat = new float[vector.Length][];
        for (int i = 0; i < vector.Length; i++)
            mat[i] = new float[] { vector[i] };
        return mat;
    }
    ...
    #region Feature Standardization

    private (float[] means, float[] stdDevs) ComputeFeatureScaling(float[][] data)
    {
        int n = data.Length, d = data[0].Length;
        float[] means = new float[d];
        float[] stdDevs = new float[d];
        for (int j = 0; j < d; j++)
        {
            float sum = 0f;
            for (int i = 0; i < n; i++)
            ...
            float variance = 0f;
            for (int i = 0; i < n; i++)
                variance += (data[i][j] - means[j]) * (data[i][j] - means[j]);
            stdDevs[j] = MathF.Sqrt(variance / n);
            if (stdDevs[j] < 1e-6f)
                stdDevs[j] = 1f;
        }
    ...
    private float[][] StandardizeData(float[][] data, float[] means, float[] stdDevs)
    {
        int n = data.Length, d = data[0].Length;
    ...
    private float[] StandardizeSample(float[] sample, float[] means, float[] stdDevs)
    {
        int d = sample.Length;
        float[] result = new float[d];
        for (int j = 0; j < d; j++)
            result[j] = (sample[j] - means[j]) / stdDevs[j];
        return result;
    }

    #endregion

    #region Basis Function Implementation

    private enum BasisFunctionType
    {
        Constant,
        HingePositive,
        HingeNegative,
    ...
    private class BasisFunction
    {
        public BasisFunctionType Type { get; }
        public int FeatureIndex { get; }     // For hinge functions
        public float Knot { get; }           // For hinge functions
        public List<BasisFunction> Components { get; }  // For interactions
        public int InteractionLevel { get; } // 0 for constant; 1 for raw hinge; sum for interactions

        // Constructor for constant and raw hinge functions.
        public BasisFunction(BasisFunctionType type, int featureIndex = -1, float knot = 0f)
        {
            Type = type;
            FeatureIndex = featureIndex;
            Knot = knot;
            Components = null;
            InteractionLevel = (type == BasisFunctionType.Constant) ? 0 : 1;
        }

        // Constructor for interaction terms (product of two basis functions).
        public BasisFunction(BasisFunction bf1, BasisFunction bf2)
        {
            Type = BasisFunctionType.Interaction;
            Components = new List<BasisFunction>();
            if (bf1.Components != null)
                Components.AddRange(bf1.Components);
            else
                Components.Add(bf1);
            if (bf2.Components != null)
                Components.AddRange(bf2.Components);
            else
                Components.Add(bf2);
            InteractionLevel = Components.Sum(b => b.InteractionLevel);
            FeatureIndex = -1;
            Knot = 0f;
        }

        /// <summary>
        /// Evaluates the basis function on a single sample.
        /// (This method is used in final prediction; caching is done separately.)
        /// </summary>
        public float Evaluate(float[] sample)
        {
            switch (Type)
            {
                case BasisFunctionType.Constant:
                    return 1f;
                case BasisFunctionType.HingePositive:
                    return MathF.Max(0, sample[FeatureIndex] - Knot);
                case BasisFunctionType.HingeNegative:
                    return MathF.Max(0, Knot - sample[FeatureIndex]);
                case BasisFunctionType.Interaction:
                    float prod = 1f;
                    foreach (var comp in Components)
                        prod *= comp.Evaluate(sample);
                    return prod;
                default:
                    return 0f;
            }
        ...
        /// <summary>
        /// Generates a unique key for caching evaluations.
        /// </summary>
        public string GetCacheKey()
        {
            switch (Type)
            {
                case BasisFunctionType.Constant:
                    return "Constant";
                case BasisFunctionType.HingePositive:
                    return $"HingePos_F{FeatureIndex}_K{Knot}";
                case BasisFunctionType.HingeNegative:
                    return $"HingeNeg_F{FeatureIndex}_K{Knot}";
                case BasisFunctionType.Interaction:
                    var compKeys = Components.Select(c => c.GetCacheKey()).OrderBy(k => k);
                    return "Interaction(" + string.Join(":", compKeys) + ")";
                default:
                    return "Unknown";
            }
        }
        ...
        public override string ToString()
        {
            switch (Type)
            {
                case BasisFunctionType.Constant:
                    return "BF_Constant";
                case BasisFunctionType.HingePositive:
                    return $"BF_HingePos_F{FeatureIndex}_K{Knot}";
                case BasisFunctionType.HingeNegative:
                    return $"BF_HingeNeg_F{FeatureIndex}_K{Knot}";
                case BasisFunctionType.Interaction:
                    return "BF_Interaction(" + string.Join("*", Components.Select(c => c.ToString())) + ")";
                default:
                    return "BF_Unknown";
            }
        }
    ...
    /// <summary>
    /// Returns the evaluation vector for a given basis function over all samples,
    /// using caching to avoid duplicate computation.
    /// </summary>
    private float[] GetEvaluations(BasisFunction bf, float[][] data)
    {
        string key = bf.GetCacheKey();
        ...
        if (bf.Type == BasisFunctionType.Constant)
        {
            for (int i = 0; i < n; i++)
                evals[i] = 1f;
        }
        else if (bf.Type == BasisFunctionType.HingePositive)
        {
            for (int i = 0; i < n; i++)
                evals[i] = MathF.Max(0, data[i][bf.FeatureIndex] - bf.Knot);
        }
        else if (bf.Type == BasisFunctionType.HingeNegative)
        {
            for (int i = 0; i < n; i++)
                evals[i] = MathF.Max(0, bf.Knot - data[i][bf.FeatureIndex]);
        }
        else if (bf.Type == BasisFunctionType.Interaction)
        {
            // For interactions, compute the product of component evaluations.
            // Start with ones.
            for (int i = 0; i < n; i++)
                evals[i] = 1f;
            foreach (var comp in bf.Components)
            {
                float[] compEvals = GetEvaluations(comp, data);
                for (int i = 0; i < n; i++)
                    evals[i] *= compEvals[i];
            }
        }

        evaluationCache[key] = evals;
        return evals;
    }
...
public class Data
{
    ...
    static float[] CreateData(int features, int dataLen, float[] minVal, float[] maxVal, int seed, float targetMax)
    {
    ...
    static float[] CreateLabels(int trainingLen, int features, float[] data, bool isLabelNorm)
    {
    ...
    static float[][] CreateNormData(int trainingLen, int features, float[] data, bool isLabelNorm)
    {
    ...
    static float LabelNorm(float output, bool invert = true) => (invert ? (1 - output) : (output));
    static float NormData(float val, float min, float max) => (((val - min) / (max - min)) * 2 - 1);

    static float Optimizer//Old
